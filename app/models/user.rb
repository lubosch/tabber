class User < ActiveRecord::Base
  ...
  def last_software(time)
    LogSoftware.joins(:software).joins("JOIN (#{last_softwares_id(24.hours.ago).to_sql}) lss ON lss.stopa=timestamp").select("name, pocet,softwareWindowName").order(:pocet).reverse_order
  end
